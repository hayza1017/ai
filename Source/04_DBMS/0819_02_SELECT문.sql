...
--3. 특정행만 출력 : WHER절(조건절)에서 비교연산자 - 같다(=), 다르다(!=, ^=, <>), >, <, >=, <=
SELECT EMPNO NO, ENAME NAME, SAL FROM EMP WHERE SAL=3000;
SELECT EMPNO NO, ENAME NAME, SAL FROM EMP WHERE SAL<>3000;
SELECT EMPNO NO, ENAME NAME, SAL FROM EMP WHERE SAL>3000;
    -- 비교연산자는 숫자, 문자, 날짜 모두 가능
    -- EX1. 사원 이름(ENAME)이 'A', 'B', 'C'로 시작하는 사원의 모든 필드 'A'<'AA'<'AAA'<'B'<'C<'CA'<'CAA'
    ...
    SELECT * FROM EMP
        WHERE HIRDATE < TO_DATE('1/01/01', 'RR/MM/DD');
    ALTER SESSION SET NLS_DATE_FORMAT = 'RR/MM/DD';

-- 4. 특정 행만 출력 : WHERE절(조건절)에서 논리연산자 : OR, AND, NOT
    -- EX1. 급여(SAL)가 2000부터 3000인 직원의 사번, 이름, 급여
    SELECT * FROM EMP WHERE 2000<=SAL AND SAL<=3000;
    -- EX2. 82년도에 입사한 사원의 모든 필드
    SELECT * FROM EMP WHERE HIREDATE>='82/01/01' AND HIREDATE<='82/01/01';
    SELECT * FROM EMP 
...
-- 5. 산술연산자(SELECT절, WHERE절, ORDER BY절)
    ...
    SELECT ENAME, SAL, SAL*12 연봉   --(3)번째
        ...
        ORDER BY 연봉; -- 연봉 순 오름차순(ASC), 내림차순(DESC)정렬 (4)번째
    -- EX. 연봉이 2400이상인 직원의 ENAME, SAL, 연봉(SAL*12+COMM)
...
SELECT ENAME || '은' || JOB FROM EMP
    -- EX. 모든 사원에 대해서 "SMITH의 연봉은 XXXXX$입니다"라고 출력하고 TITLE에는 MESSAGE.
...
--연습문제 꼭 풀기
 ...
 SELECT EMPNO "사 번", ENAME "이름", SAL "급여", JOB "업무", HIREDATE "입사일" FROM EMP;

 --5. emp 테이블에서 급여가 2000미만인 사람의 사번, 이름, 급여 출력
 SELECT EMPNO "사번", ENAME "이름", SAL "급여" FROM EMP WHERE SAL<2000;

 --6. 입사일이 81/02이후에 입사한 사람의 사번, 이름, 업무, 입사일 출력
 SELECT EMPNO "사번", ENAME "이름", JOB "업무", HIREDATE "입사일" FROM EMP WHERE HIREDATE >= '81/03/01'; -- 약간 위험할 수 있음.
 ...
 SELECT EMPNO, ENAME, JOB, DEPTNO FROM EMP WHERE JOB='SALSMAN' OR SAL>=3000;
...
-- 8. SQL 연산자(BETWEEN, IN, LIKE, IS NULL)
    --(1) 필드명 BETWEEN A AND B : 필드명이 A부터 B까지(A, B포함) A<B
        ...
        -- EX. 급여가 1500이상이거 3000이하인 사람의 모든 필드 출력
        SELECT * FROM EMP WHERE SAL BETWEEN 1500 AND 3000;
        SELECT * FROM EMP WHERE SAL BETWEEN 3000 AND 1500; --불가 A가 B보다 작아야함.
        -- EX. 82년도에 입사한 사람의 모든 필드 출력
        SELECT * FROM EMP
            WHERE HIREDATE BETWEEN TO_DATE('82/01/01', 'RR/MM/DD') AND TO_DATE('82/12/31', 'RR/MM/DD');
        -- EX. 급여가 1500미만, 3000초과인 사람의 모든 필드 출력
        SELECT * FROM EMP WHERE SAL NOT BETWEEN 1500 AND 3000;
    --(2) 필드명 IN (값1, 값2, ...값N) : 필드명 값1이거나 값2이거나, ... 값N인 경우
    ...
    --(3) 필드명 LIKE 패턴 : %(0글자이상), _(한글자)를 포함된 패턴
        ...
        --EX. 이름이 S으로 끝나는 사원의 모든 필드
        SELECT * FROM EMP WHERE ENAME LIKE '%S';
        --EX. SAL이 5으로 끝나는 사원의 모든 필드
    ...
    --(4) IS NULL(NULL인지를 검색할 때)
        ...
        --EX. 상여금을 받은 사원의 모든 필드(COMM이 NULL이 아니고 COMM이 0이 아닌 사원)
        SELECT * FROM EMP WHERE COMM!=NULL AND COMM!=0; --불가
        SELECT * FROM EMP WHERE COMM IS NOT NULL AND COMM!=0;

--9. 정렬(오름차순ASC(생략가능), 내림차순 DESC) : ORDER BY절
SELECT ENAME, SAL, HIREDATE FROM EMP ORDER BY SAL, HIREDATE DESC, ENAME;
    -- 급여 오름차순, 급여가 같으면 입사일 내림차순, 금여와 입사일이 같으면 이름 오른차순 정렬

-- ※ 형변환함수
/* 날짜형 -> 문자형 : TO_CHAR(날짜형, '패턴')
                   YYYY(년도4자리) RR(년도2자리) YY(년도 2자리) MM(월 2자리), DD(월 2자리), DY(수), DAY(수요일) HH24, HH12, AM이나PM, MI(분)SS(초)
...
SELECT ENAME, JOB, HIREDATE FROM EMP WHERE HIREDATE BETWEEN TO_DATE('81/02/20', 'RR/MM/DD') AND TO_DATE('81/05/01', 'RR/MM/DD');
 
 --5.	deptno가 10,20인 사원의 모든 정보를 출력 (단 ename순으로 정렬)
 SELECT * FROM EMP WHERE DEPTNO IN (10,20); -- ★ 모르겠음? ORDER BY ENAME DESC,
 ...
 -- 8.	입사일이81년이고 업무가 'SALESMAN'이 아닌 직원의 사번, 사원명, 입사일, 업무, 급여를 검색하시오.
 SELECT EMPNO, ENAME, HIREDATE, JOB, SAL FROM EMP WHERE TO_CHAR(HIREDATE, 'RR/MM/DD') LIKE '81%' AND JOB!='SALESMAN';
 
 -- 9.	사번, 사원명, 입사일, 업무, 급여를 급여가 높은 순으로 정렬하고, 급여가 같으면 입사일이 빠른 사원으로 정렬하시오.
