...
SELECT E.*, DNAME, LOC FROM EMP E, DEPT D WHERE E.DEPTNO=D.DEPTNO;
    ...
    SELECT ENAME, JOB, SAL, E.DEPTNO FROM EMP E, DEPT D WHERE LOC='CHICAGO' AND E.DEPTNO=D.DEPTNO;
    -- EX. 상여금(COMMM)이 있는 사원 중, SAL이 1200이상인 사원의 이름, 급여, 부서번호, 부서명, 상여(부서명순, 급여큰순으로 정렬)
...
--  탄탄 다지기
    ...
    --	ACCOUNTING 부서 소속 사원의 이름과 입사일을 입사일 최신 순으로 출력하시오
    ...
    -- EX. 모든 사원의 사번, 이름, 직책, 상사사번, 급여, 금여등금(1등급, 2등급...)
...
--  탄탄다지기 연습문제
    ...
    SELECT ENAME, SAL, GRADE, (SAL+NVL(COMM, 0))*12 ANNULAL_SAL, DNAME
        FROM EMP E, DEPT D, SALGRADE
        WHERE E.DEPTNO=D.DEPTNO AND SAL BETWEEN LOSAL AND HISAL
        ORDER BY DNAME, ANNULAL_SAL;
...
SELECT EMPNO, ENAME FROM EMP; --14행
    ...
    SELECT W. EMPNO, W.ENAME, W.MGR, M.EMPNO MNO, M.ENAME MNAME
        FROM EMP W, EMP M
        WHERE W.MGR=M.EMPNO;
    
    -- EX. 'SMITH의 상사는 FORD다' 포맷으로 모든 지원을 출력
...
-- ▶ 4. OUTER JOIN : EQUI-JOIN, NON-EQUI-JOIN, SELF-JOIN시 조건에 만족하지 않는 행까지 나타나게 하는 JOIN
 --(1) SELF-JOIN에서의 OUTTER JOIN : 조건에 배제된 행을 결과에 포함 시킬 경우, +기호를 정보가 부족한 컬럼에 붙임.
...
-- Part1(EQUI JOIN, NON EQUI JOIN)
    ...
    SELECT ENAME, JOB, SAL, DNAME
        FROM EMP E, DEPT D
        WHERE E.DEPTNO=D.DEPTNO AND LOC='NEW YORK'; -- 쓰긴 썼는데, 값이 안나오는데?
    
    --3. 보너스를 받는 사원에 대하여 이름,부서명,위치를 출력
    SELECT ENAME, DNAME, LOC
        FROM EMP E, DEPT D
        WHERE E.DEPTNO=D.DEPTNO AND COMM>0; -- 보너스 어떻게 해야하는데? 
    ...
    --6. 사번, 사원명, 업무, 급여, 급여등급, 부서명을 출력(단, 업무가 MANAGER이며 급여가 2500이상인 사원에 대하여 사번을 기준으로 오름차순정렬)
    SELECT EMPNO, ENAME, JOB, SAL, GRADE, DNAME
        FROM EMP E, DEPT D, SALGRADE
        WHERE E.DEPTNO=D.DEPTNO AND SAL BETWEEN LOSAL AND HISAL AND JOB='MANAGER' AND SAL>=2500;

-- Part2(4가지 JOIN 모두)
    --1. 이름, 급여, 업무, 직속상사명
    SELECT W.ENAME, W.SAL, M.JOB, M.ENAME
        FROM EMP W, EMP M
        WHERE W.MGR=M.EMPNO;
    
    --2. 이름, 급여, 업무, 직속상사명 . (상사가 없는 직원까지 전체 직원 다 출력. 상사가 없을 시 '없음'으로 출력)
    SELECT W.ENAME, W.SAL, M.JOB,NVL(M.ENAME, '없음') MANAGER
        FROM EMP W, EMP M
        WHERE W.MGR=M.EMPNO(+);

    --3. 이름, 급여, 부서명, 직속상사명
    SELECT W.ENAME, W.SAL, DNAME, M.ENAME MANAGER
        FROM DEPT D, EMP W, EMP M
    ...
    SELECT W.ENAME, W.SAL, GRADE, DNAME, (W.SAL+NVL(W.COMM,0)*12 ANNUAL_SAL, NVL(M.ENAME, '없음')
        FORM 
