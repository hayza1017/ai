...
drop user userid; -- 계정 삭제alter
-- 데이터 베이스로 들어가기
show databases; -- 데이터베이스들의 리스트
create database devdb; -- 새로운 데이터베이스 (devdb) 생성
show databases; -- 새로운 데이터베이스가 있는지 확인하기 위한 실행
...
-- ■■ ※ DDL ※ ■■
-- ■■■■■■■■■■■■■■
/* mySQL 타입 : numberic(n,d), varchar(n), date, datetime, timeatamp
...
select * from friend;
insert into friend (name, tel) values ('고길동', null);
insert into friend (name, tel, age) values ('하석진', '010-1111-1111', 20);
insert into friend (name, tel, age) values ('지석진', '010-3333-1111', 40);

-- ★ 파이썬 수업시간에 쓸 테이블 ★
-- DDL과 DML의 명령어는 데이터베이스 안에서만 실행
...
-- 5. 사번, 이름, 부서번호, 급여. 부서번호 순 정렬 같으면 sal 큰순
...
-- 8. 사번, 이름, 상사이름(상사가 없는 사람도 출력하되 상사가 없는 경우 ★CEO★로 출력) – oracle과 다른 문법 outer join임.
select w.pno, w.pname, ifnull(m.pname,'★CEO★') manager
	from person w left join person m
		on w.manager=m.pno ;
    
-- 8-1 사번, 이름, 상사사번(상사가 없으면 ceo로출력. ifnull함수의 매개변수의 타입이 상이해도 상관없음) – oracle과 다른 문법
...
-- 8-2 사번, 이름, 급여, 고액연봉자(4000이상) or 일반연봉자 (언제 if를 사용하나의 예)
select pno, pname, sal, if(sal>=4000, '고액연봉자', '일반연봉자') 비교
	from person;
...
select now(); -- select 절만으로도 실행 가능

-- date_format(날짜/시간필드, 포맷) => 문자형
	-- 포맷 : %Y(년도 4자리), %y(년도 2자리), %m(월 2자리), %M(월 이름), %d(일2자리)
    -- %H(24시간), %h(12시간), %p(오전, 오후), %i(분), %s(초)
select pname, date_format(hiredate, '%Y년%m원%d일') 취업날짜 from person;

-- fromat(숫자필드, 소수점자리수)
select pname, format(sal, ) sal from person;

-- oracle에서의 연결연산자 || smith는 manager입니다.
select pname, job from person; -- mysql에서의 ||는 or연산자이다
select concat(pname, '는', job, '입니다.') 문장 from person;